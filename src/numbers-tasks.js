...
function roundToPowerOfTen(num, pow) {
  const zeroCounts = 10 ** pow;
  return Math.round(num / zeroCounts) * zeroCounts;
}
...
function getSumOfDigits(num) {
  return String(num)
    .split('')
    .reduce((acc, val) => Number(val) + acc, 0);
}
...
function getCountOfOddNumbers(number) {
  const moduleNum = Math.abs(number);
  let r = 0;

  for (let i = 0; i <= moduleNum; i += 1) {
    if (i % 2 !== 0) {
      r += 1;
    }
  }
